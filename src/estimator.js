...
const covid19ImpactEstimator = (data) => {
  return {
    ...
    impact: {
      ...
      dollarsInFlight: (Math.trunc(((data.reportedCases * 10) * (2 ** factor(data)) * (data.avgDailyIncomeInUSD) * (data.avgDailyIncomePopulation)) / data.timeToElapse))
    },
    severeImpact: {
      ...
      dollarsInFlight: (Math.trunc(((data.reportedCases * 50) * (2 ** factor(data)) * (data.avgDailyIncomeInUSD) * (data.avgDailyIncomePopulation)) / data.timeToElapse))
    }
  };
};
